...
from .execute_query import get_ids_list, table_records, filters
...
def filters_search(group_list: list, filters_list: list, table_name: str, offset: int):
   ...
   """
    filter with AND logic and pagination (5 rows/page). Supports grouping via group_list.

    Input
    - group_list: [column_name_to_group_by:str, ....]  # columns to GROUP BY; may be empty for no grouping
        • Example: ["BuildingId"] on table="Units" returns one row per BuildingId group.
   ...
   res = filters( filters_list, table_name, offset)

    ...
    for t in tool_calls:
        ...
        if not t['name'] in tools_dict: # Checks if a valid tool is present
          ...
          try:
            ...
            if t['name'] == "property_TOOL":
                print(f"Calling Tool: {t['name']} with query:{t['args'].get('query', 'No query provided')}")
                
                if t['args'].get('query', None) == None:
                    result =  "query can not be None"
                else: 
                    result = tools_dict[t['name']].invoke({"query":t['args'].get('query', '')})
            
            elif t['name'] == "DEALS_TOOL":
                print(f"Calling Tool: {t['name']} with query:{t['args'].get('query', 'No query provided')}")
